...
export const Route = createFileRoute("/dashboard")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Acompanhe seu nível, ranking da comunidade e o impacto ambiental das suas coletas." },
    ],
  }),
});
...
const BADGES = [
  ...
  { icon: Flame, label: "Sequência 7 dias", earned: true, color: "bg-reward/30 text-reward-foreground" },
  { icon: Award, label: "100L Coletados", earned: false, color: "bg-secondary text-muted-foreground" },
];
  ...
  return (
    <MobileShell>
      <header className="px-5 pt-6 pb-4 flex items-center justify-between bg-gradient-hero">
        <Link to="/collect" className="size-10 rounded-full bg-card border border-border flex items-center justify-center shadow-soft">
      ...
      <section className="px-5 -mt-1">
        <div className="rounded-3xl bg-gradient-primary text-primary-foreground p-5 shadow-elevated">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-bold uppercase tracking-wider opacity-90">Seu nível</div>
      ...
      <section className="px-5 mt-4 grid grid-cols-2 gap-3">
        <MetricCard icon={Droplets} value="248.000L" label="de água protegidos" tone="water" />
        ...
        <div className="grid grid-cols-4 gap-3">
          {BADGES.map(({ icon: Icon, label, earned, color }) => (
            <div key={label} className="flex flex-col items-center text-center">
              <div className={`size-14 rounded-2xl flex items-center justify-center ${color} ${earned ? "shadow-soft" : "opacity-60"}`}>
          ...
          {LEADERBOARD.map((row, i) => (
            ...
            >
              <span className={`size-8 rounded-full flex items-center justify-center text-sm font-bold ${
                i === 0 ? "bg-gold/30 text-foreground" : i === 1 ? "bg-silver/40 text-foreground" : i === 2 ? "bg-bronze/30 text-foreground" : "bg-secondary text-muted-foreground"
              }`}>
                {i + 1}
              </span>
              <span className={`flex-1 text-sm ${row.you ? "font-bold text-primary" : "font-medium"}`}>{row.name}</span>
