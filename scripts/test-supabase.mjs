...
async function run() {
  ...
  try {
    ...
    if (n >= 4) {
      ok(`badges: ${n} registros`);
      passed++;
    } else {
      err("badges", { message: `esperado ≥4, encontrado ${n}` });
      failed++;
    }
  } catch (e) {
    err("badges seed", e);
  ...
  try {
    ...
    if (n >= 1) {
      ok(`collection_points: ${n} registro(s)`);
      passed++;
    } else {
      err("collection_points seed", { message: "nenhum ponto de coleta" });
      failed++;
    }
  } catch (e) {
    err("collection_points seed", e);
    failed++;
  }
  ...
  try {
    ...
    if (error || !data || data.length === 0) {
      ok("profiles bloqueado para anon (RLS ativo)");
      passed++;
    } else {
      err("RLS", { message: `deveria retornar 0 rows, retornou ${data.length}` });
      failed++;
    }
  } catch (_) {
    ok("profiles bloqueado para anon (RLS ativo)");
  ...
  try {
    ...
    if (!error) {
      ok("rankings acessível com service role");
      passed++;
    } else {
      err("rankings", error);
      failed++;
